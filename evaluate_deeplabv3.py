...
def evaluate_test_set():
    ...
    model_path = os.path.join(os.path.dirname(data_dir), "dbcnet_glacial_lakes", "best_deeplabv3.pth")
    
    if os.path.exists(model_path):
        model.load_state_dict(torch.load(model_path, map_location=device))
        print(f"Loaded best DeepLabV3 model from {model_path}")
    else:
    ...
    with torch.no_grad():
        pbar = tqdm(test_loader, desc="Testing DeepLabV3")
    ...
    print("\n" + "="*40)
    print("Final DeepLabV3 Evaluation on Test Set")
    print("="*40)
